...
Azure 区域:: 集群必须位于 Azure NetApp Files 可用的区域。 https://["按区域查看 Azure 产品"^]。
...
* 请注意， vNet （包括对等 VNet ）与 Azure NetApp Files 中使用的 IP 数量的默认限制为 1 ， 000 。 https://["查看 Azure NetApp Files 资源限制"^]。
+
如果您即将达到限制，您有两种选择：

+
** 您可以 https://["提交增加限制的请求"^]。如需帮助，请联系您的 NetApp 代表。
...
.步骤
. 登录 Azure 门户。
. https://["按照 Azure NetApp Files 文档注册 NetApp 资源提供商"^]。
...
.步骤
. https://["按照 Azure NetApp Files 文档从 Azure 门户创建 NetApp 帐户"^]。
...
.步骤
. https://["按照 Azure NetApp Files 文档设置自动 QoS 容量池"^]。
...
.步骤
. https://["按照 Azure NetApp Files 文档将子网委派给 Azure NetApp Files"^]。
