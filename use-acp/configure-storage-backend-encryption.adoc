...
* *内部部署Kubernet*—控制配置程序支持通过从Red ONTAP OpenShift和上游Kubernetes集群到内部ONTAP卷的NFS3和NFSv4连接进行Kerberos加密。
...
.开始之前
* 确保您已安装 link:../use/enable-acp.html["已启用Asta Control配置程序"^] 在受管集群上。
